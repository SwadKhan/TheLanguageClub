import React from "react";
import { Link } from "react-router-dom";

const CategoryLesson = ({ card }) => {
  const {
    category,
    word,
    pronunciation,
    meaning,
    example,
    image,
    part_of_speech,
    difficulty,
    when_to_say,
  } = card;

  const pronounceWord = (word) => {
    const utterance = new SpeechSynthesisUtterance(word);
    utterance.lang = "ja-JP"; // Japanese language
    window.speechSynthesis.speak(utterance);
  };

  return (
    <div className="flex flex-col justify-between bg-gradient-to-r from-blue-50 via-white to-blue-50 shadow-2xl rounded-xl overflow-hidden h-full">
      <figure className="w-full h-48">
        <img src={image} alt={word} className="w-full h-full object-cover" />
      </figure>
      <div className="p-6 flex flex-col flex-grow">
        <h2 className="text-xl lg:text-2xl font-bold text-blue-700 mb-2">
          {word} (Under {category} category)
        </h2>
        <p className="text-gray-600 italic mt-1">
          <strong>Pronunciation:</strong> {pronunciation}
        </p>
        <p className="text-gray-800 mt-2">
          <strong>Meaning:</strong> {meaning}
        </p>
        <p className="text-gray-800 mt-2">
          <strong>Part of Speech:</strong> {part_of_speech}
        </p>
        <p className="text-gray-800 mt-2">
          <strong>Difficulty:</strong> {difficulty}
        </p>
        <p className="text-gray-800 mt-2">
          <strong>When to Say:</strong> {when_to_say}
        </p>
        <p className="text-gray-800 mt-2">
          <strong>Example:</strong> {example}
        </p>
      </div>
      <div className="p-4 flex justify-between items-center">
        <button
          onClick={() => pronounceWord(word)}
          className="btn btn-primary bg-blue-500 text-white hover:bg-blue-600 px-4 py-2 rounded-md"
        >
          Pronounce Word
        </button>
      </div>
    </div>
  );
};

export default CategoryLesson;
